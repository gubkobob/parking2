...
def test_create_client(client) -> None:
    client_data = {"name": "Александр", "surname": "Белов",
...
def test_create_client_parking(client, db) -> None:
    ...
    client_parking_data = {"time_in": datetime.now() - timedelta(hours=2),
                           "client_id": 2,
                           "parking_id": 1}
    resp = client.post("/client_parkings", data=client_parking_data)

    client_parking_data2 = {"time_in": datetime.now() - timedelta(hours=3),
                           "client_id": 1,
                           "parking_id": 2}
    ...
    places = db.session.query(Parking.count_available_places).filter(Parking.id == 1).scalar()
...
def test_delete_client_parking(client, db) -> None:
    ...
    client_parking_data1 = {"client_id": 1,
                           "parking_id": 1}
    resp1 = client.delete("/client_parkings", data=client_parking_data1)

    client_parking_data2 = {"client_id": 2,
                            "parking_id": 2}
